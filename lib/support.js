 ...
 * 通过图灵机器人回答
 */
exports.tuling123 = function (keyword, cb){
    ...
    var qs = {
        ...
        userInfo: {
            apiKey: 'a7138b816b9645aba1b6188a9d452009',
                userId: '335992'
        }
    }
    ...
    request.post(options, function(err, res, body){
        ...
        if(err){
            error('tuling123 failed', err);
            return cb(err);
        }
        var data = JSON.parse(body);
        if(data.results && data.results.length>=1){
            data = data.results[0];
            var text = data.values.text;
            return cb(null, text);
        }
        return cb('tuling123 found nth.');
    });
};